...
export function calculateExposureDose(measurements, inputParams) {
  ...
  // Check Lane R integrity: Poka-yoke blank reference MUST be 0.0 mm to pass.
  // Any stain front > 0 mm means gas penetrated the sealed barrier -> FAIL.
  const laneRLength = measurements.laneR?.lengthMM ?? 0.0
  const isIntegrityPassed = laneRLength <= 0.0 && measurements.laneR?.integrity !== 'FAIL'

  if (isIntegrityPassed) {
    result.integrityStatus = 'PASS'
  } else {
    result.integrityStatus = 'FAIL'
    result.warnings.push(
      `Lane R integrity check FAILED (${laneRLength.toFixed(1)} mm stain detected). Sealed poka-yoke reference breached. Band is compromised and this scan cannot be treated as a valid dosimetry reading.`
    )
  }

  // Extract measured lengths (0.0 to 50.0 mm max scale)
  const lengthA = Math.min(50.0, measurements.laneA?.lengthMM ?? 35.0)
  const lengthB = Math.min(50.0, measurements.laneB?.lengthMM ?? 35.0)
  const lengthR = Math.min(50.0, measurements.laneR?.lengthMM ?? 35.0)
  ...
  result.calculations.lengthR = lengthR
  ...
  // Apply live humidity & temperature corrections if provided
  let tempCorrectedDose = estimatedDose
  if (inputParams?.temperatureC != null && !isNaN(inputParams.temperatureC)) {
    const tempC = parseFloat(inputParams.temperatureC)
    // Temperature correction factor according to ASTM D4599 / diffusive sampling standard:
    // Reference temperature: 25°C (298.15 K)
    const tempK = tempC + 273.15
    const tempCorrectionFactor = Math.pow(tempK / 298.15, 1.5)
    tempCorrectedDose = estimatedDose / tempCorrectionFactor

    result.calculations.temperatureC = tempC
    result.calculations.tempCorrectionFactor = tempCorrectionFactor

    result.formulasUsed.push({
      name: 'Temperature Compensation (ASTM D4599)',
      formula: 'Dose_T = Dose / ((T + 273.15) / 298.15)^1.5',
      variables: {
        'Ambient Temperature': `${tempC.toFixed(1)} °C (${tempK.toFixed(1)} K)`,
        'Reference Temperature': '25.0 °C (298.15 K)',
        'Correction Factor (kT)': tempCorrectionFactor.toFixed(4),
        'Compensated Dose': `${tempCorrectedDose.toFixed(2)} ppm·hr`,
      },
      explanation: 'Gas diffusivity increases with ambient temperature. Applying ASTM D4599 diffusive compensation accurately normalizes the dose.',
    })
  }

  if (inputParams?.relativeHumidity != null && !isNaN(inputParams.relativeHumidity)) {
    const rh = parseFloat(inputParams.relativeHumidity)
    result.calculations.relativeHumidity = rh
    result.formulasUsed.push({
      name: 'Ambient Humidity Validation',
      formula: `Relative Humidity = ${rh.toFixed(0)}% RH`,
      variables: {
        'Measured Ambient RH': `${rh.toFixed(0)}%`,
        'Calibration Domain': `${PROJECT_CONFIG.humidityMin}% – ${PROJECT_CONFIG.humidityMax}% RH`,
        'Ratiometric Compensation': 'Active (Lane A / Lane B)',
      },
      explanation: 'High Humident (Lane A) and Low Humident (Lane B) dual-channel ratio cancels out ambient moisture interference.',
    })

    if (rh < PROJECT_CONFIG.humidityMin || rh > PROJECT_CONFIG.humidityMax) {
      result.warnings.push(
        `Ambient humidity (${rh.toFixed(0)}% RH) is outside standard test envelope (${PROJECT_CONFIG.humidityMin}%–${PROJECT_CONFIG.humidityMax}% RH). Ratiometric correction applied.`
      )
    }
  }

  // Final dose
  result.dose = Math.round(tempCorrectedDose * 100) / 100
  result.success = true
