...
export class BandDetector {
  ...
  detectRotatedReactionFronts(bandRegion, fiducials, lanes, angleRad) {
    ...
    const frontRPixels = laneRFront !== null ? laneRFront : defaultSpanPixels * 0.95
    ...
    // Integrity check: PASS if within standard scale range (<= 50.0 mm)
    const isIntegrityPass = lengthRMM <= 50.0
  ...
  sampleRotatedLaneFront(bandRegion, laneOffset, ux, uy, vx, vy, spanPixels) {
    ...
    // Default to ~35 mm on 50 mm scale (70% of span)
  ...
  createAnnotatedImage(results, bandRegion, fiducials, lanes, angleDeg, angleRad) {
    ...
    if (results.measurements) {
      const lengthA = Math.min(50.0, results.measurements.laneA.lengthMM)
      const lengthB = Math.min(50.0, results.measurements.laneB.lengthMM)
      const lengthR = Math.min(50.0, results.measurements.laneR.lengthMM)
      ...
      ctx.fillText(`Integrity: ${lengthR.toFixed(1)} mm [PASS]`, startX + 6, laneTopR + laneH * 0.6)
    }
